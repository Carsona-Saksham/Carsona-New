...
const AddBrands = Vue.component('AddBrands', {
    template: `
      <div>
    ...
    <h3 class="text-center font-weight-bold mt-5">Add Brand</h3>
    <div class="form d-flex justify-content-center align-items-center pb-5">
      <form @submit.prevent="addBrand">
        <div class="mb-3">
          <label class="form-label">Brand Name</label>
          <input type="text" class="form-control" v-model="newBrand.brand" />
        </div>
        <div class="mb-3">
  ...
  reader.onload = () => {
    this.newBrand.poster = reader.result;
  };
  console.log(this.newBrand.poster)
  reader.readAsDataURL(file);
},
      addBrand: function() {
          axios.post('/api/add_brand', {
            brand: this.newBrand.brand,
            poster: this.newBrand.poster
      },{headers: { 
        'Content-Type': 'application/json',
        'Authorization': 'Bearer ' + localStorage.getItem('access_token') }})
          .then(response => {
            this.messages.success = 'Your  is being added';
            this.title_m = '';
            this.poster_m = '';
